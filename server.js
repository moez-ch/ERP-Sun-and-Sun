...
// Durable copies of every generated price quote / contract (Ahmet: "downloading
// isn't enough — save it somewhere"). Files are served back via the History
// download links. Kept out of git via .gitignore.
const STORAGE_DIR = path.join(__dirname, "generated_docs");
if (!fs.existsSync(STORAGE_DIR)) fs.mkdirSync(STORAGE_DIR);
try { db.exec(`ALTER TABLE price_quotes ADD COLUMN stored_file TEXT`); } catch {}
try { db.exec(`ALTER TABLE contracts   ADD COLUMN stored_file TEXT`); } catch {}

// Write generated bytes to STORAGE_DIR as "<prefix>_<rowId>.<ext>"; returns the
// filename (recorded in stored_file) or null on failure (never fatal).
function persistGenerated(prefix, rowId, ext, bytes) {
  try {
    const fname = `${prefix}_${rowId}.${ext}`;
    fs.writeFileSync(path.join(STORAGE_DIR, fname), bytes);
    return fname;
  } catch (e) { console.warn("[persist]", e.message); return null; }
}

// Persist the delivered contract artifact (pdf or docx), record it on the row,
// and post the Odoo chatter note. Called once per generation with the final bytes.
function finalizeContractDoc(rowId, ext, bytes, data, user) {
  try {
...
// Optional: only active when ODOO_URL/DB/USER/API_KEY are set in .env. Used to
// drop a chatter note on the matching company ("Ziynet sent a Price Quote").
let _odooUid = null;
async function odooExec(model, method, args = [], kwargs = {}) {
  const url = process.env.ODOO_URL, dbName = process.env.ODOO_DB,
        user = process.env.ODOO_USER, pass = process.env.ODOO_API_KEY || process.env.ODOO_PASSWORD;
  if (!url || !dbName || !user || !pass) return null; // feature off until creds exist
  const base = url.replace(/\/+$/, "");
  const rpc = async (service, rmethod, rargs) => {
    const r = await fetch(base + "/jsonrpc", {
      method: "POST", headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ jsonrpc: "2.0", method: "call", params: { service, method: rmethod, args: rargs }, id: Date.now() }),
    });
    const j = await r.json();
    if (j.error) throw new Error(j.error.data?.message || j.error.message || "odoo rpc error");
    return j.result;
  };
  if (!_odooUid) _odooUid = await rpc("common", "authenticate", [dbName, user, pass, {}]);
  if (!_odooUid) throw new Error("Odoo auth failed");
  return rpc("object", "execute_kw", [dbName, _odooUid, pass, model, method, args, kwargs]);
}
...
app.post("/contracts/generate", authenticate, async (req, res) => {
  ...
  if (isHtmlTemplate) {
    try {
      ...
      const cInfo = db.prepare("INSERT INTO contracts (template_id, template_name, data, created_by, created_by_name) VALUES (?,?,?,?,?)")
        .run(templateId, row.name, JSON.stringify(data), req.user.id, req.user.name || req.user.email);
      finalizeContractDoc(cInfo.lastInsertRowid, "pdf", pdfBytes, data, req.user);
  ...
  if (isDocxtemplater) {
    try {
      ...
      const cInfo = db.prepare("INSERT INTO contracts (template_id, template_name, data, created_by, created_by_name) VALUES (?,?,?,?,?)")
        .run(templateId, row.name, JSON.stringify(data), req.user.id, req.user.name || req.user.email);
      if (returnWord) {
        finalizeContractDoc(cInfo.lastInsertRowid, "docx", docxBuf, data, req.user);
      ...
      finalizeContractDoc(cInfo.lastInsertRowid, "pdf", pdfBuf, data, req.user);
      res.setHeader("Content-Type", "application/pdf");
      res.setHeader("Content-Disposition", "attachment");
  ...
  try {
    ...
    if (returnWord) {
      ...
      fs.unlinkSync(docxPath);
      finalizeContractDoc(cInfo.lastInsertRowid, "docx", buf, data, req.user);
      res.setHeader("Content-Type", "application/vnd.openxmlformats-officedocument.wordprocessingml.document");
      res.setHeader("Content-Disposition", "attachment");
      return res.send(buf);
    }
...
app.get("/contracts/report", authenticate, (req, res) => {
  ...
  const rows = db.prepare(
    `SELECT id, template_id, template_name, data, created_by_name, created_at, stored_file FROM contracts ${where} ORDER BY created_at DESC`
  ).all(...params);
  ...
  const parsed = rows.map(r => {
    ...
    return { id: r.id, template_name: r.template_name, prepared_by: r.created_by_name, prepared_for: d.party2_name || "", value, created_at: r.created_at, has_file: !!r.stored_file, fields };
  });
...
app.get("/pricing/report", authenticate, (req, res) => {
  ...
  const rows = db.prepare(
    `SELECT id, mode, client_name, program_name, data, created_by_name, created_at, stored_file FROM price_quotes ${where} ORDER BY created_at DESC`
  ).all(...params);
...
// GET /pricing/file/:id, /contracts/file/:id — download the stored copy of a
// generated document. Same visibility as the reports: admins get any, everyone
// else only their own.
...
app.get("/contracts/file/:id", authenticate, serveStored("contracts"));
...
app.post("/pricing/generate-program", authenticate, async (req, res) => {
  ...
  try {
    ...
    // the PDF for later download, and drop a chatter note on the Odoo company.
    try {
      const presRow = db.prepare("SELECT name FROM program_presentations WHERE design_id=?").get(targetDesignId);
      const info = db.prepare("INSERT INTO price_quotes (mode, client_name, program_name, data, created_by, created_by_name) VALUES ('program',?,?,?,?,?)")
...
app.post("/pricing/generate", authenticate, async (req, res) => {
  ...
  try {
    ...
    try {
      const presRow = db.prepare("SELECT name FROM program_presentations WHERE design_id=?").get(targetDesignId);
      const info = db.prepare("INSERT INTO price_quotes (mode, client_name, program_name, data, created_by, created_by_name) VALUES ('quote',?,?,?,?,?)")
