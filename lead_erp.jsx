...
// a leave/unpaid-leave day counts as "partial" (worked alongside the leave) once the employee also logged
// actual in/out clock times and a specific leave range — otherwise it's treated as a full day off
function attIsPartialLeave(ent) {
  const type = ent?.type || "";
  return (type === "paid_leave" || type === "unpaid_leave")
    && attRangeMins(ent?.leaveStart, ent?.leaveEnd) != null
    && !!ent?.in && !!ent?.out;
}
function attDayDiff(ent) {
  ...
  if (!partial) {
    if (type === "holiday" || type === "paid_leave" || type === "unpaid_leave") return null; // excluded — full day off, leave hours are tracked as a record only
...
function AttendanceView({ colors, font, lang, onBack }) {
  ...
  // but a leave day where the employee also logged real clock times + a leave range ("partial leave") still counts as worked
  const attFlags = ent => {
    const type = ent?.type || "", halfWith = ent?.halfWith || "";
    const partial = attIsPartialLeave(ent);
    return { type, halfWith, partial,
      isOff:  type === "holiday" || (type === "half_holiday" && !!halfWith) || ((type === "paid_leave" || type === "unpaid_leave") && !partial),
  ...
  function empWeekDiff(emp, wd) {
    let total = null;
    wd.forEach(d => {
      const v = attDayDiff(data[emp]?.[attDK(d)]);
  ...
  function printMonthlyReport() {
    ...
    const detailSections = employees.map(emp => {
      const workedCount = allDays.filter(d => { const e = data[emp]?.[attDK(d)]; return e?.in && e?.out && !attFlags(e).isOff; }).length;
      ...
      const dayRows = allDays.map(d => {
        ...
        const { type, isOff, isHalf, partial } = attFlags(ent);
        ...
        const dayLabel = isHalf ? `${dayName}<br><small style="font-style:italic;color:#888">${LEAVE_LABELS.half_holiday}</small>`
          : partial ? `${dayName}<br><small style="font-style:italic;color:#888">${attLeaveNote(ent)}</small>`
          : dayName;
        return `<tr>
          <td>${attFD(d)}</td><td>${dayLabel}</td>
          <td>${ent.in||"—"}</td><td class="${arrDelta==null?"neutral":arrDelta>0?"neg":"pos"}">${arrLabel}</td>
          <td>${ent.out||"—"}</td><td class="${isHalf||depDelta==null?"neutral":depDelta<0?"neg":"pos"}">${depLabel}</td>
...
<div class="footer">${tr("Work schedule: 08:30–18:30 · 10h expected per working day","Mesai saatleri: 08:30–18:30 · Günlük beklenen: 10 saat")}</div>
  ...
  function printEmployeeDetail(emp) {
    ...
    const rows = allDays.map(d => {
      ...
      const arrLabel = arrDelta == null ? "—" : arrDelta === 0 ? tr("On time","Tam") : arrDelta > 0 ? `+${attAbsStr(arrDelta)} ${tr("late","geç")}` : `-${attAbsStr(arrDelta)} ${tr("early","erken")}`;
      const depLabel = isHalf ? "—" : (depDelta == null ? "—" : depDelta === 0 ? tr("On time","Tam") : depDelta > 0 ? `+${attAbsStr(depDelta)}` : `-${attAbsStr(depDelta)} ${tr("early","erken")}`);
      return { d, dk, ent, type, isOff, isHalf, partial, arrDelta, depDelta, dayDiff, arrLabel, depLabel };
    });

    const workedCount = rows.filter(r => !r.isOff && r.ent.in && r.ent.out).length;
    const empB = empBreakdown(emp, allDays);

    const bodyRows = rows.map(({ d, ent, type, isOff, isHalf, partial, arrDelta, depDelta, dayDiff, arrLabel, depLabel }) => {
      const dayName = FULL_DAYS[d.getDay() - 1];
      ...
      const dayLabel = isHalf ? `${dayName} <small style="font-style:italic;color:#888">(${LEAVE_LABELS.half_holiday})</small>`
        : partial ? `${dayName} <small style="font-style:italic;color:#888">(${attLeaveNote(ent)})</small>`
        : dayName;
      ...
      return `<tr>
        ...
        <td>${ent.out || "—"}</td>
        <td class="${isHalf||depDelta==null?"neutral":depDelta<0?"neg":"pos"}">${depLabel}</td>
        <td class="${dayDiff==null?"neutral":dayDiff>=0?"pos":"neg"}">${attFmt(dayDiff)}</td>
      </tr>`;
    }).join("");
  ...
  return (
    <div style={{ animation: "slideIn .3s ease" }}>
      ...
      {screen === "dashboard" && (
        ...
        {selectedEmp && (() => {
          const allDays = weeks.flat();
          const workedDays = allDays.filter(d => { const e = data[selectedEmp]?.[attDK(d)]; return e?.in && e?.out && !attFlags(e).isOff; });
                    ...
                    {weeks.map((wd, wii) => (
                      <tr key={wii}>
                        ...
                        {[1,2,3,4,5].map(dow => {
                          ...
                          const depDelta  = isHalf ? null : (depMins  != null ? depMins  - 1110 : null);
                          ...
                          return (
                            <td key={dow} style={cellSt}>
                              <div style={{ borderRadius:8, border:`1px solid ${isHalf ? `${colors.accent}50` : colors.border}`, background:colors.bg, overflow:"hidden", minHeight:82 }}>
                                {/* Date header */}
                                <div style={{ padding:"3px 6px", borderBottom:`1px solid ${colors.border}`, fontSize:9, color: isHalf ? colors.accent : colors.textMuted, fontFamily:font, textAlign:"center", fontWeight:700, background:colors.surface }}>
                                  {attFD(d)}{isHalf ? ` · ${tr("½ Holiday","½ Tatil")}` : ""}
                                </div>
                                {/* Morning / Arrival */}
                                ...
                                {/* Evening / Departure */}
                                <div style={{ padding:"6px 8px", borderBottom: partial ? `1px solid ${colors.border}50` : "none", background: noData || isHalf ? "transparent" : depDelta != null && depDelta < 0 ? `${colors.danger}08` : `${colors.success}06` }}>
                                  <div style={{ fontSize:9, color:colors.textDim, fontFamily:font, marginBottom:2 }}>↓ {tr("Departure","Çıkış")}</div>
                                  <div style={{ fontSize:12, fontWeight:700, color: noData ? colors.textDim : (isHalf ? colors.text : depColor), fontFamily:font }}>{ent.out || "—"}</div>
                                  {!noData && !isHalf && depLabel && <div style={{ fontSize:9, color:depColor, fontFamily:font, marginTop:1 }}>{depLabel}</div>}
                                </div>
                                {/* Partial leave note */}
                                {partial && (
                                  <div style={{ padding:"5px 8px", background:`${LEAVE_COLORS[type]}10` }}>
                                    <div style={{ fontSize:9, fontWeight:700, color:LEAVE_COLORS[type], fontFamily:font, textAlign:"center" }}>{attLeaveNote(ent)}</div>
                                  </div>
                                )}
                              </div>
                            </td>
...
- Türkçe konuş, ama gerekirse İngilizceye geç`,
  ...
  const EMPTY_CONTRACT_DATA = () => ({
    ...
    down_payment_deadline: "", sb1ddl: "", sb1ddl_type: "", success_bonus_2_deadline: "", sb1ddl_2_type: "",
  ...
  const fetchMondayBoard = async (boardType = mondayBoardType) => {
    ...
    try {
      ...
      // Tags come back from the board query directly (workspace-level)
      const allTags = data?.data?.tags || [];
      setMondayTags(allTags.sort((a, b) => a.name.localeCompare(b.name)));
    } catch (e) {
  ...
  useEffect(() => {
    ...
    if (view === "contracts") {
      ...
      fetch("/dropdown-options", { headers: { Authorization: `Bearer ${token}` } })
        .then(r => r.json()).then(setDropdownExtras).catch(() => {});
      fetch("/contracts/companies", { headers: { Authorization: `Bearer ${token}` } })
    ...
    <div style={{ display: "flex", height: "100vh", fontFamily: font, background: colors.bg, color: colors.text, overflow: "hidden" }}>
      ...
      <div style={{ flex: 1, overflow: "auto", padding: isMobile ? 12 : 24, minWidth: 0 }}>
        ...
        {view === "monday" && (() => {
          ...
          return (
            <div style={{ animation: "slideIn .3s ease" }}>
              ...
              {newTemplateModal && (
                <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.65)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center" }}>
                  <div style={{ background: colors.surface, borderRadius: 12, padding: 28, width: 500, maxWidth: "92vw", maxHeight: "60vh", overflowY: "auto", border: `1px solid ${colors.border}` }}>
        ...
        {view === "contracts" && (() => {
          ...
          const addDropdownOption = async (dropdownKey, value) => {
            if (!value?.trim()) return;
            const token = localStorage.getItem("sns_token");
            const r = await fetch("/dropdown-options", { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify({ dropdown_key: dropdownKey, value: value.trim() }) });
            const d = await r.json();
            if (d.ok) setDropdownExtras(prev => ({ ...prev, [dropdownKey]: [...(prev[dropdownKey] || []), { id: d.id, value: value.trim() }] }));
            setAddingDropdown(null); setNewDropdownVal("");
          };
          const deleteDropdownOption = async (id, dropdownKey) => {
            const token = localStorage.getItem("sns_token");
            await fetch(`/dropdown-options/${id}`, { method: "DELETE", headers: { Authorization: `Bearer ${token}` } });
            setDropdownExtras(prev => ({ ...prev, [dropdownKey]: (prev[dropdownKey] || []).filter(o => o.id !== id) }));
          };
          const saveEditOption = async () => {
            if (!editingDropdownOption || !editingDropdownOption.value.trim()) return;
            const { id, value, dropdownKey } = editingDropdownOption;
            const token = localStorage.getItem("sns_token");
            const r = await fetch(`/dropdown-options/${id}`, { method: "PUT", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify({ value: value.trim() }) });
